...
st.title("📊 Mini Dashboard Interactivo")

# --- Datos de ejemplo ---
df = pd.DataFrame({
    "Nombre": ["Ana", "Luis", "Carla", "Pedro", "Marta", "Juan", "Lucia"],
    "Edad": [25, 30, 22, 28, 35, 40, 27],
    "Ciudad": ["Madrid", "Barcelona", "Sevilla", "Valencia", "Bilbao", "Madrid", "Sevilla"],
    "Ventas": [1000, 1500, 1200, 800, 2000, 1700, 900]
})

# --- Filtros por usuario ---
st.sidebar.header("Filtros")
ciudad_filtro = st.sidebar.multiselect("Filtrar por ciudad", options=df["Ciudad"].unique(), default=df["Ciudad"].unique())
edad_min = st.sidebar.slider("Edad mínima", min_value=0, max_value=100, value=0)
edad_max = st.sidebar.slider("Edad máxima", min_value=0, max_value=100, value=100)

# Filtrar datos
df_filtrado = df[(df["Ciudad"].isin(ciudad_filtro)) & (df["Edad"] >= edad_min) & (df["Edad"] <= edad_max)]
...
with col1:
    ...
    st.dataframe(df_filtrado, height=250)

with col2:
    st.subheader("Tabla avanzada (AgGrid)")
    AgGrid(df_filtrado)

# --- Gráfico ---
st.subheader("Gráfico de Ventas por Ciudad")
fig = px.bar(df_filtrado, x="Ciudad", y="Ventas", color="Nombre", text="Ventas")
st.plotly_chart(fig, use_container_width=True)

# --- Información resumida ---
with st.expander("Resumen de ventas"):
    st.write(f"**Total Ventas:** {df_filtrado['Ventas'].sum()}")
    st.write(f"**Promedio Ventas:** {df_filtrado['Ventas'].mean():.2f}")
